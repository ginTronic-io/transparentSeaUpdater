// dfu.js — from devanlai/webdfu (https://github.com/devanlai/webdfu)
var dfu = {};
...
(function() {
        ...
        return navigator.usb.getDevices().then(devices => {
            ...
            for (let device of devices) {
                ...
                for (let intf of interfaces) {
                    matches.push(new dfu.Device(device, intf));
                }
            }
            return matches;
        });
    ...
    dfu.Device.prototype.logProgress = function(done, total) {
        if (typeof total === 'undefined') { console.log(done); } else { console.log(done + '/' + total); }
    };
    ...
    dfu.Device.prototype.close = async function() {
        try { await this.device_.close(); } catch (error) { console.log(error); }
    };
    ...
    dfu.Device.prototype.readDeviceDescriptor = function() {
        ...
        return this.device_.controlTransferIn({
            ...
            request: GET_DESCRIPTOR, value: wValue, index: 0
        }, 18).then(result => {
    ...
    dfu.Device.prototype.readStringDescriptor = async function(index, langID) {
        ...
        const setup          = { requestType: "standard", recipient: "device", request: GET_DESCRIPTOR, value: wValue, index: langID };
        var result = await this.device_.controlTransferIn(setup, 1);
        if (result.status == "ok") {
    ...
    dfu.Device.prototype.readInterfaceNames = async function() {
        ...
        for (let index of allStringIndices) {
            try { strings[index] = await this.readStringDescriptor(index, 0x0409); }
            catch (error) { console.log(error); strings[index] = null; }
        }
    ...
    dfu.parseInterfaceDescriptor = function(data) {
        return {
            ...
            iInterface: data.getUint8(8), descriptors: []
        };
    };
    ...
    dfu.parseSubDescriptors = function(descriptorData) {
        ...
        while (remainingData.byteLength > 2) {
            ...
            if (bDescriptorType == DT_INTERFACE) {
                ...
                let desc = { bLength, bDescriptorType, data: descData };
    ...
    dfu.Device.prototype.do_download = async function(xfer_size, data, manifestationTolerant) {
        ...
        try { await this.download(new ArrayBuffer([]), transaction++); }
        catch (error) { throw "Error during final DFU download: " + error; }
        ...
        try { await this.device_.reset(); } catch (error) {
