// dfuse.js — from devanlai/webdfu (https://github.com/devanlai/webdfu)
// dfu.js must be included before this file.
...
(function() {
    ...
    dfuse.parseMemoryDescriptor = function(desc) {
        ...
        let segments = [];
        const sectorMultipliers = { ' ': 1, 'B': 1, 'K': 1024, 'M': 1048576 };
    ...
    dfuse.Device.prototype.dfuseCommand = async function(command, param, len) {
        ...
        const commandNames = { 0x00: "GET_COMMANDS", 0x21: "SET_ADDRESS", 0x41: "ERASE_SECTOR" };
        ...
        else               { throw "Don't know how to handle data of len " + len; }
        try { await this.download(payload, 0); }
        catch (error) { throw "Error during DfuSe command " + commandNames[command] + ": " + error; }
        let status = await this.poll_until(state => (state != dfu.dfuDNBUSY));
        if (status.status != dfu.STATUS_OK) {
            throw "DfuSe command " + commandNames[command] + " failed";
        }
    };

    dfuse.Device.prototype.getSegment = function(addr) {
        if (!this.memoryInfo || !this.memoryInfo.segments) throw "No memory map information available";
    ...
    dfuse.Device.prototype.getSectorStart = function(addr, segment) {
        if (typeof segment === 'undefined') segment = this.getSegment(addr);
    ...
    dfuse.Device.prototype.erase = async function(startAddr, length) {
        ...
        while (addr < endAddr) {
            if (segment.end <= addr) segment = this.getSegment(addr);
            if (!segment.erasable) {
    ...
    dfuse.Device.prototype.do_download = async function(xfer_size, data, manifestationTolerant) {
        if (!this.memoryInfo || !this.memoryInfo.segments) throw "No memory map available";
        ...
        try { await this.poll_until(state => (state == dfu.dfuMANIFEST)); }
    ...
    dfuse.Device.prototype.do_upload = async function(xfer_size, max_size) {
        ...
        if (state != dfu.dfuIDLE) await this.abortToIdle();
        await this.dfuseCommand(dfuse.SET_ADDRESS, startAddress, 4);
